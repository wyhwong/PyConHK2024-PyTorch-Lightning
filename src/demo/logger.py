...
def get_logger(
    logger_name: str,
    streaming_log_level: int = demo.env.STREAMING_LOG_LEVEL,
    file_log_level: int = demo.env.FILE_LOG_LEVEL,
    log_filepath: Optional[str] = demo.env.LOG_FILEPATH,
) -> logging.Logger:
    """Function to create a logger object with the specified name and log levels.

    Args:
        logger_name (str): Name of the logger object.
        streaming_log_level (int, optional): Log level for console logging. Defaults to demo.env.STREAMING_LOG_LEVEL.
        file_log_level (int, optional): Log level for file logging. Defaults to demo.env.FILE_LOG_LEVEL.
        log_filepath (Optional[str], optional): Path to the log file. Defaults to demo.env.LOG_FILEPATH.
    ...
    formatter = logging.Formatter(fmt=demo.env.LOG_FMT, datefmt=demo.env.LOG_DATEFMT)
